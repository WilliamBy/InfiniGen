...
# Copied from modelscope.models.bart.modeling_bart._expand_mask
...
class LlamaAttention(nn.Module):
    ...
    def gen_mask_2(self, attn, th):
        ...
        for i in range(capa, tgt_len):
            ...
            if i != (tgt_len-1):
                ...
                # lru
                    ...
                    idx = idx * fetch_mask[:,:i+1,:int(i/2)]
        ...
        fetch_mask = torch.where(fetch_mask==1, 0, prune_value)

        sp = torch.count_nonzero(fetch_mask > -10000).item() /heads/(tgt_len*(tgt_len+1)/2)
        fetch_mask = fetch_mask.unsqueeze(0)
        return fetch_mask, sp
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from modelscope import AutoTokenizer, LlamaForCausalLM
